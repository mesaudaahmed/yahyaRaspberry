...
def getSetting(table:str):
    ...
    return d
#retrun data tag into dictonnary 
def getData(table:str,tag):
    d={}
    query=" SELECT * FROM "+table+" WHERE tag = ?"
    selector1=cursor.execute(query ,(tag,))
    selector=selector1.fetchall()[0]
    for index,row in enumerate(cursor.description) :
        d[row[0]]=selector[index]
    return d
...
# insertdata("esp08",1001,datetime.now(),"60","1","1","10#20","10#20","10#20",12)
